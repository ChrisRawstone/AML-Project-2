...
class PiecewiseLinearCurve(torch.nn.Module):
    def __init__(self, c_0, c_1,f=None, G=None, n_intervals=10,device='cpu'):
    ...
    def length(self):
        ...
        for i in range(1, self.n_intervals + 1):
            ...
            norm2 += torch.sum((f_c_s - f_c_s_minus_1) ** 2).sqrt()
        return norm2
    ...
    def finite_diff(self):
        ...
        for i in range(1, self.n_intervals + 1):
            f_c_s = self.f(c[:, i].unsqueeze(0))
            f_c_s_minus_1 = self.f(c[:, i - 1].unsqueeze(0))
            norm2 += torch.sum((f_c_s - f_c_s_minus_1) ** 2)/dt
        return norm2
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        for i in range(args.num_curves):
            ...
            f = lambda x: model.decoder(x).mean
            curve = PiecewiseLinearCurve(z0.T, z1.T,f=f, G=model.decoder.pull_back_metric, n_intervals=args.num_t, device=device).to(device)
